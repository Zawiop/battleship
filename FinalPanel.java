...
import java.util.concurrent.TimeUnit;
 ...
 static int[][] loc1, loc2, loc3, loc4, loc5;
 static int[] data1, data2, data3, data4, data5;
 ...
 private class setShips implements ActionListener {
    ...
    */
    public void actionPerformed(ActionEvent e) {
      ...
      for (int i = 0; i < 10; i++) {
         for (int j = 0; j < 10; j++) {
             fin[i][j] = loc2[i][j] + loc1[i][j];
         }
     }
     for (int a = 0; a< 10; a++) {
      for (int j = 0; j < 10; j++) {
          fin[a][j] = fin[a][j] + loc3[a][j];
      }
   }
      for (int b = 0; b < 10; b++) {
         for (int j = 0; j < 10; j++) {
             fin[b][j] = fin[b][j] + loc4[b][j];
         }
     }
     for (int c = 0; c < 10; c++) {
      for (int j = 0; j < 10; j++) {
          fin[c][j] = fin[c][j] + loc5[c][j];
      }
  }
  for (int d = 0; d < 10; d++) {
   for (int j = 0; j < 10; j++) {
    ...
    System.out.println("Next Time, Place All Ships");
    System.exit(0);
}

  
     // System.out.println(Arrays.deepToString(fin).replace("], ", "]\n"));
